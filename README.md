...
0.4.7 supersedes the insert-class conclusion below. The capture include-list and the configuration-filter behavior in this section stay.
...
## Software instance apply (0.4.7)

Re-Execute of 0.4.6 on the live PDIs still failed as `sbridge.worker`. The Data Execution was again 15 selected / 0 sent / cancelled. The outbox for all 15 rows already had `name`, `installed_on`, `software`, and `ref_keys` (the 0.4.6 capture include-list worked). The apply error was `insert into cmdb_software_instance returned no sys_id (canCreate=true): empty name, installed_on`. `cmdb_sam_sw_install` does not exist on PDI2, so the 0.4.6 alternate-class insert never ran. DETAILED computers and their Record Mapping rows are on PDI2. No software rows landed. `sbridge.worker` is still not admin.

**Why name and installed_on were empty at insert.** They were not missing from the outbox. Two apply steps dropped them after that, and the 0.4.6 diagnostic read the GlideRecord only after `insert()` returned null (a failed insert often clears the record, so both mandatory fields look empty even when `setValue` held them):

1. Translate can blank `installed_on` before the child rewrite sees it. An explicit xref miss, or reference handling `null_and_flag`, stores `''`. `_localizeChildValues` skips an empty value, so it never restores the outbox sys_id or the computer Record Mapping. An implicit miss keeps the **source** sys_id. That id is not a CI on PDI2. The business-key fallback queries the stamped class (`cmdb_ci_win_server`) rather than `cmdb_ci`, so a computer stored as `cmdb_ci_computer` is a miss. The reference element then clears `installed_on`. A before rule that builds `name` from `software` clears `name` when `software` is also a source sys_id that does not exist here.
2. The hardcoded software reference is `cmdb_software_product_model`. On this PDI that table is not the dictionary target (`cmdb_ci_spkg`), and a hit on `cmdb_ci_spkg` was discarded because the row was not in the product-model table. The dangling `software` sys_id is what makes the before rule abort with `canCreate=true` and no platform message. The columns are the right ones: the diagnostic only adds a field when `isValidField` is true.

**Fix.** Apply seals the working copy, then stamps `name` and `installed_on` onto the GlideRecord **before** `insert`, and reads them back before the call. `installed_on` is the PDI2 computer: Record Mapping by source sys_id with no `source_table` filter (an empty xref cache entry is not trusted), then a unique `cmdb_ci` name. A source sys_id that is not a local CI is left off the record instead of being set and cleared. `software` is kept only when the dictionary reference contains the row; otherwise apply creates that package as the integration user (`cmdb_ci_spkg` when that is the reference) or omits the dangling id. `name` is copied back from the outbox when the working copy lost it, and it is stamped last. The insert table is `cmdb_software_instance`. `cmdb_sam_sw_install` is not selected. The metadata writer allow-list is unchanged. Role-only Allow rules and cross-scope create/read/write cover `cmdb_software_instance` and `cmdb_ci_spkg`. A new fix script opens Can read / Can create / Can update on those tables. It does not grant admin. If insert still returns no sys_id, the error includes `payload name=… installed_on=…` and `held before insert name=… installed_on=…`, and says when a before rule cleared them during insert.

**Verify (do not deploy from this change set).** Install 0.4.7 on both peers. `now-sdk build` must succeed before that install. The new fix script is **Open software instance application access**. Re-Execute only the Path A software-instance movement, as `sbridge.worker`, after the computer Record Mapping exists. Do not install `cmdb_sam_sw_install` for this test.
