/**
 * Fallback confirmation page. The form button uses a dialog instead.
 * Calls the execution controller only. Never redirects to a blank form.
 */
function sbridgeConfigUrl(value) {
    var id = value == null ? '' : String(value)
    if (!/^[0-9a-f]{32}$/i.test(id)) return 'x_33764_sbridge_movement_config_list.do'
    return 'x_33764_sbridge_movement_config.do?sys_id=' + id
}
...
if (out && out.ok && out.dex_id && /^[0-9a-f]{32}$/i.test(String(out.dex_id))) {
    ...
    if (out && out.dex_id && /^[0-9a-f]{32}$/i.test(String(out.dex_id))) {
        response.sendRedirect('x_33764_sbridge_data_execution.do?sys_id=' + out.dex_id)
    } else {
        response.sendRedirect(sbridgeConfigUrl(configId))
    }
}
